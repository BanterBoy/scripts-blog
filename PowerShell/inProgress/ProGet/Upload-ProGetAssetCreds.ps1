<#
 ...
 .Parameter ChunkSize
  ...
  .Parameter Username
  username to access Asset resource

  .Parameter Password
  password to access Asset resource
...
function Upload-ProGetAsset {
    ...
    #modification to add user authentification
    ...
    if ($fileInfo.Length -le $chunkSize) {
        Invoke-WebRequest -Method Post -Uri $targetUrl -InFile $FileName -Headers $headers
    }
    else {
        ...
        try {
            ...
            for ($index = 0; $index -lt $totalParts; $index++) {
                ...
                $req.headers["Authorization"] = "$basicAuthValue"
                $req.Method = 'POST'
