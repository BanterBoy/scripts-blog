...
Function GetComputerWMIInfo {
	...
	Try {
		...
		$Nics = $Results | Where-Object { $_.ipenabled -eq $True }
		$Results = $Null
	}
...
$AppPartitions = $Forest.ApplicationPartitions | Sort-Object
...
$CrossForestReferences = $Forest.CrossForestReferences | Sort-Object
...
$SPNSuffixes = $Forest.SPNSuffixes | Sort-Object
...
$UPNSuffixes = $Forest.UPNSuffixes | Sort-Object
...
$Domains = $Forest.Domains | Sort-Object
If ($Null -eq $Domains) {
	WriteWordLine 0 0 "<None>"
}
Else {
...
$Sites = $Forest.Sites | Sort-Object
If ($Null -eq $Sites) {
	WriteWordLine 0 0 "<None>"
}
Else {
...
ForEach ($Domain in $Domains) {
	...
	If ($? -and $Null -ne $DomainInfo) {
		...
		If ($? -and $Null -ne $Users) {
			...
			$Results = $Users | Where-Object { $_.Enabled -eq $False }
			...
			$Results = $Users | Where-Object { $_.LockedOut -eq $True }
			...
			$Results = $Users | Where-Object { $_.PasswordExpired -eq $True }
			...
			$Results = $Users | Where-Object { $_.PasswordNeverExpires -eq $True }
			...
			$Results = $Users | Where-Object { $_.PasswordNotRequired -eq $True }
...
<button class="btn" type="submit" onclick="window.open('/PowerShell/scripts/activeDirectory/ADDS_Inventory.ps1')">
...
<a class="github-button" href="https://github.com/BanterBoy/scripts-blog/issues/new?title=ADDS_Inventory.ps1&body=There is a problem with this function. Please find details below." data-show-count="true" aria-label="Issue BanterBoy/scripts-blog on GitHub">Issue</a>
