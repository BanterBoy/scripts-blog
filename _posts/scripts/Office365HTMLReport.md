...
Foreach ($Admin in $Admins) {
    ...
    if ($Null -ne ($admin.assignedlicenses.SkuID)) {
        $Licensed = $True
    }
    else {
        $Licensed = $False
...
Foreach ($LooseUser in $LooseUsers) {
    ...
    if ($Null -ne ($LooseUser.assignedlicenses.SkuID)) {
        $LicensedLoose = $true
    }
    else {
        $LicensedLoose = $false
...
<button class="btn" type="submit" onclick="window.open('/PowerShell/scripts/information/Office365HTMLReport.ps1')">
...
<a class="github-button" href="https://github.com/BanterBoy/scripts-blog/issues/new?title=Office365HTMLReport.ps1&body=There is a problem with this function. Please find details below." data-show-count="true" aria-label="Issue BanterBoy/scripts-blog on GitHub">Issue</a>
