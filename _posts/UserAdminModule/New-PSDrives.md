...
permalink: /useradminmodule/shell/new-psdrives/
---
...
`New-PSDrives` is similar to `New-GitDrives` but skips the initial existence check and simply enumerates each child directory under the selected root, creating a PowerShell drive for every folder it finds.
...
When prompted, choose the base directory whose immediate child folders you want to expose as PSDrives.
...
<button class="btn" type="submit" onclick="window.open('/PowerShell/UserAdminModule/Shell/Public/New-PSDrives.ps1')">
